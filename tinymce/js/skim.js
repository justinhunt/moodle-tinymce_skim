...
var tinymce_skim_Dialog = {
         getdivfornote : function(stylekey) {
            var rawstyle = "<div style='display: inline; background: @backcolor; color: @forecolor; font-weight: @bold; border: @border @border-color 1px; font-style: @italic; text-decoration: @underline'>";
            var defstyle='';
            ...
            var underlinevalue = (styles.indexOf('underline')>-1 ? 'underline' : 'none');
        ...
        convertrtf : function() {
            var content, retcontent, currentpage,parentpage,parentnote,parentstyle,currentnote,currentstyle,lines;
            ...
            if (lines.length > 1) {
                    ...
                    if (stylekey){
                        //process old note
                           if(parentstyle=='' && currentstyle !=''){
                                ...
                                if(parentnote.indexOf(currentnote)>-1 && parentpage==currentpage){
                                    parentnote = parentnote.replace(currentnote, '&nbsp;' + currentstyle + currentnote + '</div>&nbsp;');
                                }else{
                                    retcontent += '<p>Page ' + parentpage + '<br/>' + parentstyle + parentnote + '</p><br /></div>';
                                    parentstyle = currentstyle;
                                    parentnote = currentnote;
                                    parentpage = currentpage;
                                }
                ...
                //tidy up the final parent and current notes
                 ...
                 retcontent += 'Page ' + parentpage + '<br/>' + parentstyle + parentnote + '</div>';
